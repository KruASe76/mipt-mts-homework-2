...
public class BookService {
    ...
    public void partialUpdate(BookPatchRequest request) {
        ...
        request.courseId().ifPresent(this::verifyCourseId);

        repository.update(
                current
                        .withTitle(request.title().orElse(current.title()))
                        .withAuthor(request.author().orElse(current.author()))
                        .withCourseId(request.courseId().orElse(current.courseId()))
        );
    }
