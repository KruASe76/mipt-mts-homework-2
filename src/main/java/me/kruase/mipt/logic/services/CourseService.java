...
public class CourseService {
    ...
    public void partialUpdate(CoursePatchRequest request) {
        ...
        request.universityId().ifPresent(this::verifyUniversityId);

        repository.update(
                current
                        .withName(request.name().orElse(current.name()))
                        .withCredits(request.credits().orElse(current.credits()))
                        .withUniversityId(request.universityId().orElse(current.universityId()))
        );
    }
